...
import com.staker.main.util.UIUtil;
...
public class SmoothCheckBox extends View implements Checkable {
  ...
  private static final int COLOR_CHECKED = Color.parseColor("#1296db");
...
//    TypedArray ta = getContext().obtainStyledAttributes(attrs, R.styleable.SmoothCheckBox);
    int tickColor = Color.WHITE;//对勾颜色
    mAnimDuration = 300;//动画持续时间

    mCheckedColor = COLOR_CHECKED;//选中时整个填充颜色
    mFloorColor = Color.parseColor("#DFDFDF");//未选中时边框颜色
    mUnCheckedColor =  Color.parseColor("#ffffff");//未选中时填充颜色
    mStrokeWidth = 5;//未选中时边框宽度
//    ta.recycle();
  ...
  public void setCheckedNoLisetener(boolean checked){
    mChecked = checked;
    reset();
    invalidate();
  }
  ...
  private int measureSize(int measureSpec) {
    int defSize = UIUtil.dp2px(getContext(), DEF_DRAW_SIZE);
